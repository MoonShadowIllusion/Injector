...
namespace inject {
    bool ManualMapDll(
        HANDLE hProc, BYTE *pSrcData, SIZE_T FileSize, bool SEHExceptionSupport, DWORD fdwReason, LPVOID lpReserved) {
        ...
        // File header
        if(!WriteProcessMemory(hProc, pTargetBase, pSrcData, 0x1000,
        ...
        if(!WriteProcessMemory(hProc, pShellcode, reinterpret_cast<LPCVOID>(Shellcode), 0x1000, nullptr)) {
...
#pragma optimize("", off)
    void __stdcall Shellcode(MANUAL_MAPPING_DATA *pData) {
        ...
        auto *pOpt = &reinterpret_cast<IMAGE_NT_HEADERS *>(
                          pBase + reinterpret_cast<IMAGE_DOS_HEADER *>((uintptr_t)pBase)->e_lfanew)
                          ->OptionalHeader;
        ...
        if(pData->SEHSupport) {
            ...
            if(excep.Size) {
                unsigned long long _ = reinterpret_cast<unsigned long long>(pBase);
                _ += excep.VirtualAddress;
                if(!_RtlAddFunctionTable(
                       reinterpret_cast<_RUNTIME_FUNCTION *>(_),
